...
import torch
from neural_helper import NeuralNet
...
class Server:
    ...
    def fedAVG(self, models: dict[str, NeuralNet]):
        ...
        for id, model in models.items():
            for global_param, user_param in zip(
                self.global_model.parameters(), model.parameters()
            ):
...
if __name__ == "__main__":
    ...
    server = Server(
        time_to_wait=time, num_rounds=rounds, port=port, model=global_model
    )
    server.run()
