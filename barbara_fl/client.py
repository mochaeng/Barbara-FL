...
from neural_helper import NetHelper
...
class Client:
    def __init__(
        self,
        id: str,
        user_data: client_helper.UserData,
        device,
        host="localhost",
        server_port=50052,
    ) -> None:
    ...
    def prepare_model_to_send(self, net_helper: NetHelper) -> bytes:
        data = {"header": "<MODEL>", "id": self.id, "model": net_helper.model}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--server-port",
        type=int,
        help="The port for the FL server",
        default=50052,
    )
    parser.add_argument(
        "--client-id",
    ...
    train_location_path = (
        f"./FLdata/train/mnist_train_client{str(client_id)}.json"
    )
    ...
    client = Client(
        client_id, user_data, device=device, server_port=server_port
    ).start()
